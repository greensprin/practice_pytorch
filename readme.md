...
$$0 \leqq Sigmoid(x) \leqq -1$$

ネイピア数は微分がしやすいという特徴がある。

ロジスティック回帰分析でも使われている模様

## tanh関数

$$ tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}} $$

$$-1 \leqq tanh(x) \leqq -1$$

Sigmoid関数よりも値域が広い

こちらもネイピア数で構成されるため微分がしやすい(簡単な式になる)

Sigmoid関数より勾配が大きく出るため、学習を高速化でき

勾配消失問題への対策も期待された

ただ、実際にはReLUのほうが勾配消失への対策として効果が高かった。

現在では、-1 ~ 1あでの値域である特徴を生かして出力層として使われている
